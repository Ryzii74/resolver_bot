...
module.exports = (tasks) => {
    return async (msg) => {
        ...
        if (words.length === 1) {
            return await resolverWithOneWordRunner(msg, dictionaryArraySource, tasks);
        }

        const [word1, word2] = words;
        if (word1[word1.length - 1] === '!') {
            return await resolverRunner(msg, wordSource(word1.replace('!', '')), associationsSource(word2), tasks, secondWordFormatter);
        }

        await resolverRunner(msg, associationsSource(word1), associationsSource(word2), tasks, bothWordsFormatter);
    };
};
