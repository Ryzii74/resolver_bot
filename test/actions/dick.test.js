...
describe('raschlen', () => {
    ...
    test('не выполняет слишком сложные вычисления', async () => {
        const res = await sendCommand('хренотенька1 хренотенька1 хренотенька1 хренотенька1 хренотенька1 хренотенька1 хренотенька1 хренотенька1 хренотенька1 хренотенька1');
        expect(res).toHaveLength(1);
        expect(res[0][0]).toBe('Слишком много вариантов для перебора');
    });
});
